	...
	if (msg.sType == DUI_MSGTYPE_BUTTONDOWN)
	{
		...
		if (sender_parent->GetName() == _T("OptionsLayout"))
		{
			if (strName == _T("addNewOption"))//添加新标签和内容
			{
				...
				std::wstring index = std::to_wstring(index_addOption);
				...
				std::string index = std::to_string(index_addOption);
#endif
...
void MainWndFrame::InitWindow()
{
	...
	CTabBarUI* tabUI = new (std::nothrow)CTabBarUI;
	if (!tabUI) return;
	_optionsLayout->AddAt(tabUI, 0);
	_tabBarUI = tabUI;
}
